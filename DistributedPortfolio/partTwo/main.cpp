...
int main(int argc, char** argv) {
  ...
  std::vector<std::string> wholePoem = readPoem(); //Load Poem
  ...
  { // loop through different nodes
    if(rank == i){ //when we get to ourselves
      int poemLine = -1;
            //Head node sends out all the lines
      MPI_Recv(&poemLine, 1, MPI_INT, src, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
      
      if (poemLine == -1) 
        continue; //just in case something goes wrong

      std::cout << i <<". We are node: " << i << std::endl; // Pretty Output
      std::string line = wholePoem[poemLine]; 
      std::cout  <<" "<< i <<". Original: " <<  line << std::endl;

      std::string word;
      std::vector<std::string> wordsVector; //Separate Words
      ...
      wordsVector.emplace_back(word) ; //deals with last word

      std::random_shuffle ( wordsVector.begin(), wordsVector.end() ); // Jumbling

      std::cout << " " << i << ". Shuffled: " ;
      for (std::string word : wordsVector)
        std::cout << word; //Show they are truly jumbled.
